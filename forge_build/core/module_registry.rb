...
module ForgeBuild
  module Core
    ...
    class ModuleRegistry
      Entry = Struct.new(:id, :name, :version, :division, :description, :factory, keyword_init: true) do
        def to_h
          { id: id, name: name, version: version, division: division, description: description }
        end
      end
      ...
      def register(id:, name:, version:, division: nil, description: '', &factory)
        key = id.to_sym
        ...
        @entries[key] = Entry.new(id: key, name: name, version: version, division: division,
