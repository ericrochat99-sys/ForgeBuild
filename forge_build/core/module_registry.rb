...
module ForgeBuild
  module Core
    ...
    class ModuleRegistry
      Entry = Struct.new(:id, :name, :version, :category, :divisions, :description, :icon, :factory, keyword_init: true) do
        def to_h
      ...
      def register(id:, name:, version:, category: 'Building Assembly', divisions: [], description: '', icon: nil, &factory)
        key = id.to_sym
        ...
        @entries[key] = Entry.new(id: key, name: name, version: version, category: category,
                                  ...
                                  description: description.to_s, icon: icon || id.to_s, factory: factory)
      end
